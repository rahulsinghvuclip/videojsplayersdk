(function (e, t) {
  ...
  r.async = true;
  r.src = '//d24n15hnbwhuhn.cloudfront.net/libs/amplitude-3.4.0-min.gz.js';
  r.onload = function () {
  ...
  var d = ['init', 'logEvent', 'logRevenue', 'setUserId', 'setUserProperties', 'setOptOut', 'setVersionName',
    ...
    'logRevenueV2', 'regenerateDeviceId', 'logEventWithTimestamp', 'logEventWithGroups'
  ];
...
amplitude.getInstance('videojs').init('3053c88a4bee016f9e02a274a8d297d3');
window.vivid || (function (window) {
  ...
  var MAX_SCREEN_WIDTH = "320", DEFAULT_WIDTH = "176";
  var videoDownloaded = false, a = [], lastUptoSeek = [], totalSeek = 0, n = 0, timeoutID, buffdetails = [],
    player = null, loadTime, clickButton, initialization, maindata, sou;
  var previousTime = 0;
  var currentTime = 0;
  var seekStart = null;
  ...
  var VideoJSPlayer = function (config) {
    ...
    $.ajax({
      ...
      url: 'http://64.62.204.62:9091/VideojsBackEnd/getDetails?cid=' + config.cid,

      // data:JSON.stringify({ "videos": [{ "contentId": config.cid, "count": 1, "identifier": "VIDEO_CONTAINER" }] }),
      ...
      success: function (data, textStatus, jqXHR) {
        ...
        console.log(obj);
        var source = [];
        var realData = obj.VIDEO_CONTAINER.list["0"];

        var format1 = (($(window).width()) < MAX_SCREEN_WIDTH) ? DEFAULT_WIDTH : MAX_SCREEN_WIDTH;

        var _3gp = "";
        var _mov = "";
        var hls = realData.videoInformation.hls.streamingProfile.url;
        if (typeof (realData.videoInformation.nonhls.find(obj => obj.format === '6' + format1)) !== 'undefined') {
          _3gp = realData.videoInformation.nonhls.find(obj => obj.format === '6' + format1).profiles[0].cdnUrl;
        }
        if (typeof (realData.videoInformation.nonhls.find(obj => obj.format === '7' + format1)) !== 'undefined') {
          _mov = realData.videoInformation.nonhls.find(obj => obj.format === '7' + format1).profiles[0].cdnUrl;
        };
        source = [{
          src: hls,
        ...
        console.log(source);
        var k = realData.thumbnailData.tcid16x9 + ".jpg";
        var poster = "https://vuclipi-a.akamaihd.net/p/tthumb192x144/v2/d-1/" + k;
        _oThis.init(config, source, realData.subtitles, poster);
      }
  ...
  VideoJSPlayer.prototype.init = function (config, source, tracks, poster) {
    var _oThis = this;
    ...
    if (!!document.createElement('video').canPlayType) {
      if (config) {
        this.configuration = {
          ...
          'poster': (config && config.poster !== null && config.poster !== undefined) ? config.poster : poster,
